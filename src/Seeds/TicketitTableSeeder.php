...
namespace Umark\Ticketit\Seeds;
...
class TicketitTableSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        foreach ($this->statuses as $name => $color) {
            $status = \Umark\Ticketit\Models\Status::create([
                'name'  => $name,
                'color' => $color,
            ]);
        }
        ...
        foreach ($this->categories as $name => $color) {
            $category = \Umark\Ticketit\Models\Category::create([
                'name'  => $name,
                'color' => $color,
            ]);
        ...
        foreach ($this->priorities as $name => $color) {
            $priority = \Umark\Ticketit\Models\Priority::create([
                'name'  => $name,
                'color' => $color,
            ]);
        }
        $categories_qty = \Umark\Ticketit\Models\Category::count();
        $priorities_qty = \Umark\Ticketit\Models\Priority::count();
        $statuses_qty = \Umark\Ticketit\Models\Status::count();
        ...
        for ($u = 1; $u <= $this->users_qty; $u++) {
            ...
            for ($t = 1; $t <= $tickets_qty; $t++) {
                ...
                $category = \Umark\Ticketit\Models\Category::find($rand_category);
                ...
                $ticket = new \Umark\Ticketit\Models\Ticket();
                ...
                for ($c = 1; $c <= $comments_qty; $c++) {
                    ...
                    $comment = new \Umark\Ticketit\Models\Comment();
